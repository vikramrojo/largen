...
export const emit_probe = guard((args = {}) => {
  const { kind = 'computed', pages, html, selectors, properties, steps, assertions, themes,
    themeAttribute, themeClass, themeStorage, viewport, timeout } = args
  try {
    const document = buildProbe({
      kind, pages, html, selectors, properties, steps, assertions, themes,
      themeAttribute, themeClass, themeStorage, viewport, timeout,
    })
    return ok({
      kind,
      document,
      ...
      caveats: [
        ...
        'Prefer `themeStorage` over the attribute override. Given the key the page reads, ' +
          'the probe writes it before load and lets the page apply the theme itself, the way ' +
          'it does for a user. Overriding the attribute can only move the outputs we know ' +
          'about, and a page that writes its palette inline on <html> to avoid a flash of the ' +
          'wrong theme has an output that beats every author rule.',
        'Setting the attribute is verified but that is not the same as the theme being in ' +
          'effect. Where the two can differ — a class-themed page, a palette pinned inline — ' +
          'the probe refuses to report values rather than returning a mix of both themes.',
        'Each row records what was actually observed: the attribute, the class, the resolved ' +
          'color-scheme, any custom properties pinned inline, and how the theme was driven.',
      ],
    })
  } catch (error) {
    return fail(error.message)
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        themeStorage: { type: 'string', description: 'The localStorage key the page reads its theme from. Preferred: the probe writes it before load and the page applies the theme itself, completely, instead of the probe overriding one of its outputs. Restored afterwards.' },
        viewport: {
          type: 'object',
