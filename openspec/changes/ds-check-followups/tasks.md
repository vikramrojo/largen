...
- [x] 1.1 Add the coined-tag classifier to `genai/lint.js`: a conservative
...
- [x] 1.2 Confirm both surfaces report it with no second implementation:
...
- [x] 1.2a Add the structural dark-rule check to `genai/lint.js` (error): any
...
- [x] 1.2b Add the structural size-variant check to `genai/lint.js` (error):
...
- [x] 1.3 Add the coined-tag rule, the reference-tier guidance, and the
...
- [x] 2.1 Measure what the SDK already rejects: script a wrong-typed, an
      ...
      passed-through.
- [x] 2.2 Close the measured gap in `guard()` with a schema-driven validator
      ...
      passes.
- [x] 2.3 Build the outcome-eval harness `site/test/evals.mjs`: scenarios
...
- [x] 3.1 Run `npx largen build` and confirm the build id is unchanged (no
